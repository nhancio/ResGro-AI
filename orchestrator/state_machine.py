...
def transition(state: OperatorState, event: Transition) -> OperatorState:
    ...
    table: dict[tuple[OperatorState, Transition], OperatorState] = {
        (OperatorState.NEW, "upload_data"): OperatorState.DATA_LOADING,
        (OperatorState.DATA_LOADING, "data_loaded"): OperatorState.DATA_LOADED,
        (OperatorState.DATA_LOADED, "start_deepdive"): OperatorState.DEEPDIVE_RUNNING,
        # Allow direct deepdive from NEW for backwards compat
