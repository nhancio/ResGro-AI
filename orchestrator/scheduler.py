"""
Scheduling: legacy review tick (`run_review` → `campaign_review.contract_pipeline`) + Resgro helpers (`review_due_at`, `is_due`).
...
def review_due_at(setup_completed_iso: str) -> str:
    """`REVIEW_DELAY_DAYS` after setup completion (Resgro)."""
...
def tick_placeholder() -> dict:
    """Called by cron; load due jobs from Redis/DB (Resgro stub)."""
