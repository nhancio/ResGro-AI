...
def _prepare_orders(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    discount_col = "Customer discounts from marketing | (funded by you)"
    if discount_col in orders.columns:
        orders[discount_col] = pd.to_numeric(orders[discount_col], errors="coerce")
        orders["_has_promo"] = orders[discount_col] < 0
    else:
        orders["_has_promo"] = False
    if fee_col in orders.columns:
        orders["_has_ad"] = orders[fee_col] < 0
    else:
        orders["_has_ad"] = False
    return orders
...
def _rollup_metrics(g: pd.DataFrame) -> dict[str, Any]:
    ...
    if "_has_ad" in g.columns:
        out["ad_penetration"] = round(float(g["_has_ad"].mean()), 4)
    if "_has_promo" in g.columns:
        out["promo_penetration"] = round(float(g["_has_promo"].mean()), 4)
    return out
