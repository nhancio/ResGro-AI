...
Reuses cloud_app CSV parsing modules via dummy-streamlit pattern (same approach as resgro_runner.py).
"""
...
# Duplicated from resgro_runner.py to avoid cross-agent imports.
# ---------------------------------------------------------------------------
...
def _marketing_breakdown(
    ...
    """Corporate vs Resgro marketing breakdown using promotion + sponsored data."""
    ...
    try:
        ...
        if combined_table is not None and not combined_table.empty:
            ...
            rename_map = {True: "Resgro", False: "Corporate", "True": "Resgro", "False": "Corporate"}
