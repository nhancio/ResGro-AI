...
def _build_html(sections: dict[str, Any], operator_id: str) -> str:
    ...
    ops_time = sections.get("ops_time", {})
    ...
    warn = (summary or {}).get("data_warning")
    if warn:
        parts.append(
            f'<div class="section" style="background:#fff8e6;border:1px solid #f0d090;border-radius:10px;padding:14px 18px;margin-bottom:16px;">'
            f'<p style="margin:0;color:#5c4a00;font-size:0.95em;"><strong>Data note:</strong> {warn}</p></div>'
        )
    ...
    <div class="tabs">
        ...
        <button class="tab-btn" data-tab="product-mix">Product Mix</button>
        <button class="tab-btn" data-tab="marketing">Marketing</button>
        <button class="tab-btn" data-tab="operations">Operations</button>
        <button class="tab-btn" data-tab="store-quality">Store Quality</button>
    </div>
    """)
    ...
    parts.append('<div id="product-mix" class="tab-panel">')
    parts.append(_section_product_mix(product_mix))
    parts.append('</div>')
    ...
    parts.append('<div id="store-quality" class="tab-panel">')
    parts.append(_section_store_quality(ops_store, ops_time))
    parts.append('</div>')
...
def _section_marketing(mkt: dict) -> str:
    ...
    if corp_promo:
        parts.append('<h3>Corporate vs Resgro — Promotions</h3>')
        ...
        parts.append(_chart_pie(labels, values, "Promo Sales: Corporate vs Resgro"))

    corp_sp = mkt.get("corporate_vs_todc_sponsored", [])
    if corp_sp:
        parts.append('<h3>Corporate vs Resgro — Sponsored Listings</h3>')
...
def _section_product_mix(pm: dict) -> str:
    if not pm:
        return '<div class="section"><h2>Product Mix</h2><p><em>No product mix data uploaded.</em></p></div>'
    parts = ['<div class="section"><h2>Product Mix</h2>']

    # Summary
    summary_rows = [
        ("Total Items Listed", f"{pm.get('total_items_listed', 0):,}"),
        ("Total Gross Sales", f"${pm.get('total_gross_sales', 0):,.2f}"),
        ("Total Units Sold", f"{pm.get('total_sold', 0):,.0f}"),
        ("Total Discounts Given", f"${pm.get('total_discounts', 0):,.2f}"),
        ("Total Item Errors", f"{pm.get('total_item_errors', 0):,.0f}"),
        ("Total Error Charges", f"${pm.get('total_error_charges', 0):,.2f}"),
    ]
    parts.append(_kv_table(summary_rows))

    # Top sellers
    top = pm.get("top_sellers", [])
    if top:
        parts.append('<h3>Top Selling Items</h3>')
        parts.append('<div class="table-scroll">')
        parts.append(_table(top[:15], {
            "item_name": "Item", "gross_sales": "Gross Sales", "total_sold": "Units Sold",
            "discounts": "Discounts", "total_errors": "Errors", "error_rate_pct": "Error Rate %"
        }, money_cols=["gross_sales", "discounts"], number_cols=["total_sold", "total_errors"]))
        parts.append('</div>')

        # Bar chart of top 8
        top8 = top[:8]
        parts.append(_chart_bar(
            [_short_name(r["item_name"], 20) for r in top8],
            [r["gross_sales"] for r in top8],
            "Top Items — Gross Sales ($)", color=_BRAND_700
        ))

    # Popular items
    popular = pm.get("popular_items", [])
    if popular:
        parts.append('<h3>DoorDash Popular Items</h3>')
        parts.append(_table(popular[:10], {
            "item_name": "Item", "gross_sales": "Gross Sales", "total_sold": "Units Sold",
        }, money_cols=["gross_sales"], number_cols=["total_sold"]))

    # Error-prone items
    errors = pm.get("top_error_items", [])
    if errors:
        parts.append('<h3>Most Error-Prone Items</h3>')
        parts.append('<div class="table-scroll">')
        parts.append(_table(errors[:12], {
            "item_name": "Item", "total_errors": "Errors", "error_charges": "Error Charges",
            "total_sold": "Units Sold", "error_rate_pct": "Error Rate %"
        }, money_cols=["error_charges"], number_cols=["total_errors", "total_sold"]))
        parts.append('</div>')

    # By store
    by_store = pm.get("by_store", [])
    if by_store:
        parts.append('<h3>Product Mix by Store</h3>')
        parts.append(_table(by_store, {
            "Store name": "Store", "items": "Items", "gross_sales": "Gross Sales",
            "total_sold": "Units Sold", "total_errors": "Errors", "error_rate_pct": "Error Rate %"
        }, money_cols=["gross_sales"], number_cols=["items", "total_sold", "total_errors"]))

    parts.append('</div>')
    return "\n".join(parts)


def _section_store_quality(ops_store: dict, ops_time: dict) -> str:
    parts = ['<div class="section"><h2>Store Quality Scorecard</h2>']

    # Store scorecard table
    scorecard = ops_store.get("store_scorecard", [])
    if scorecard:
        parts.append('<h3>Store Performance Overview</h3>')
        parts.append('<div class="table-scroll">')
        col_map = {}
        for c in ["Store Name", "Merchant Supplied ID", "Total Delivered or Picked Up Orders",
                   "Missing/Incorrect %", "Total Cancellation Rate %",
                   "Average Avoidable Dasher Wait", "Average Delivery Time (ASAP)",
                   "Uptime %", "Average Rating", "Percentage of Loved"]:
            if any(c in r for r in scorecard):
                short = c.replace("Total Delivered or Picked Up Orders", "Delivered Orders") \
                         .replace("Average Avoidable Dasher Wait", "Avg Wait (min)") \
                         .replace("Average Delivery Time (ASAP)", "Avg Delivery (min)") \
                         .replace("Merchant Supplied ID", "Store #") \
                         .replace("Percentage of Loved", "Loved %")
                col_map[c] = short
        parts.append(_table(scorecard, col_map))
        parts.append('</div>')
    else:
        parts.append('<p><em>No store scorecard data. Upload the Operations Quality (by Store) zip.</em></p>')

    # Downtime
    downtime_cats = ops_store.get("downtime_categories", {})
    if downtime_cats:
        total_dt = ops_store.get("total_downtime_minutes", 0)
        parts.append(f'<h3>Downtime Breakdown ({total_dt:,} total minutes)</h3>')
        parts.append(_chart_pie(list(downtime_cats.keys()), list(downtime_cats.values()), "Downtime by Category"))

        dt_by_store = ops_store.get("downtime_by_store", [])
        if dt_by_store:
            parts.append(_table(dt_by_store, {
                "Store name": "Store", "downtime_minutes": "Downtime (min)"
            }, number_cols=["downtime_minutes"]))

    # Cancellation categories
    cancel_cats = ops_store.get("cancellation_categories", {})
    if cancel_cats:
        parts.append('<h3>Cancellation Breakdown</h3>')
        parts.append(_chart_pie(list(cancel_cats.keys()), list(cancel_cats.values()), "Cancellation Categories"))

    # Error type breakdown
    error_types = ops_store.get("error_type_breakdown", {})
    if error_types:
        parts.append('<h3>Error Type Breakdown</h3>')
        parts.append(_chart_pie(list(error_types.keys()), list(error_types.values()), "Missing/Incorrect Error Types"))

    # Ops time — item quality ranking
    item_quality = ops_time.get("item_quality_ranking", [])
    if item_quality:
        parts.append('<h3>Item Quality Ranking (from Ops Time data)</h3>')
        col_map = {}
        for c in ["item_name", "Gross Item Sales", "Item Volume", "Total Item Missing or Incorrect Errors",
                   "Item Missing/Incorrect %", "Total Item Error Charges"]:
            if any(c in r for r in item_quality):
                short = c.replace("item_name", "Item") \
                         .replace("Gross Item Sales", "Sales ($)") \
                         .replace("Item Volume", "Volume") \
                         .replace("Total Item Missing or Incorrect Errors", "Errors") \
                         .replace("Item Missing/Incorrect %", "Error %") \
                         .replace("Total Item Error Charges", "Charges ($)")
                col_map[c] = short
        parts.append('<div class="table-scroll">')
        parts.append(_table(item_quality[:15], col_map,
                            money_cols=["Gross Item Sales", "Total Item Error Charges"],
                            number_cols=["Item Volume", "Total Item Missing or Incorrect Errors"]))
        parts.append('</div>')
