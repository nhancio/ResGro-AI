...
def export_to_excel(dd_table1, dd_table2, ue_table1, ue_table2, 
    ...
    """Export all tables to an Excel file with sheets: Summary Tables, Store-Level Tables, and Corporate vs Resgro"""
    ...
    # Sheet 3: Corporate vs Resgro Tables
    if corporate_todc_table is not None and not corporate_todc_table.empty:
        ws_corporate = wb.create_sheet("Corporate vs Resgro")
        current_row = 1
        
        # Add Combined Corporate vs Resgro table
        ...
        corporate_export['Campaign'] = corporate_export['Campaign'].apply(
            lambda x: 'Corporate' if x == False else ('Resgro' if x == True else str(x))
        )
        corporate_export = corporate_export.set_index('Campaign')
        
        current_row = add_table_to_sheet(ws_corporate, "Combined: Corporate vs Resgro", corporate_export, current_row)
        ...
        if promotion_table is not None and not promotion_table.empty:
            ...
            current_row = add_table_to_sheet(ws_corporate, "Promotion: Corporate vs Resgro", promo_export, current_row)
        ...
        if sponsored_table is not None and not sponsored_table.empty:
            ...
            current_row = add_table_to_sheet(ws_corporate, "Sponsored Listing: Corporate vs Resgro", sponsored_export, current_row)
