...
from .plan_generator import generate_plan, generate_plan_from_analysis
    ...
    try:
        ...
        for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
            if row_idx == 1 or not row:
                continue
            # Matches combined_report_agent CAMPAIGN_MAPPINGS_HEADERS:
            # Store ID, Minimum Subtotal, Slot Tags, Campaign Name, Status
            if len(row) < 4:
                continue
            store_id = str(row[0] or "").strip()
            try:
                min_subtotal = float(row[1] or 0)
            except (TypeError, ValueError):
                min_subtotal = 0.0
            slot_tags = str(row[2] or "").strip()
            campaign_name = str(row[3] or "").strip()
            status = str(row[4] or "Pending").strip() if len(row) > 4 else "Pending"
            store_name = ""
...
"""
    with tempfile.TemporaryDirectory(prefix="marketingreco_fin_") as td:
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(f"FINANCIAL_DETAILED pipeline failed: {detail}")
...
"""
    ...
    if proc.returncode != 0:
        detail = (proc.stderr or "").strip() or (proc.stdout or "").strip() or f"exit {proc.returncode}"
        raise RuntimeError(f"marketingreco auto mode failed: {detail}")
    ads_csv = _parse_subprocess_ads_csv(proc.stdout or "")
...
def run(
    ...
    *,
    ...
    # Try full analysis first (richer data → better recommendations)
    full_analysis: dict[str, Any] | None = None

    if deepdive_report is not None and "sections" in deepdive_report:
        full_analysis = deepdive_report
    else:
        analysis_file = _deepdive_analysis_path(operator_id)
        if analysis_file.is_file():
            full_analysis = json.loads(analysis_file.read_text(encoding="utf-8"))

    if full_analysis is not None:
        plan, ads_plan = generate_plan_from_analysis(full_analysis, budget_cap=budget_cap)
        out = _save_plan(operator_id, plan)
        out["ads_plan"] = ads_plan
        return out

    # Fallback: thin DeepDiveReport (legacy compat)
